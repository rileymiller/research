...
% Since hyperref and cite do not completely get along, the template now recommends using natbib:
...
% 1) do not use the hyperref package
...
\csmfigure{lda2vec}{figures/lda2vec_diagram}{3.65in}{\emph{lda2vec} Model Architecture \cite{moody2016mixing}}
...
\csmfigure{num_topics}{figures/num_topics}{6.5in}{Number of Topics and Topic Coherence \cite{stevens2012exploring}}
...
\csmfigure{top10-pi}{figures/top10-pi}{6.5in}{Top 10 Requesters before Preprocessing }
\newpage
...
\csmfigure{title-post-top10-pi}{figures/title-post-top10-pi}{6.5in}{Title -- Top 10 Requesters after Preprocessing }
\newpage
...
\csmfigure{description-post-top10-pi}{figures/description-post-top10-pi}{6.5in}{Description -- Top 10 Requesters after Preprocessing }
...
In the Title corpus distribution chart shown in \ref{fig:title-50-pi}, the relevance of the top words\
in comparison with the rest of the corpus showing that the top words account for well over a third of\
the corpus which will effect the topics that are extracted by the topic models.
...
In further depth, there are stark trends present in both the UMass and UCI topic coherence charts \
for the LSI on the Title corpus shown in \ref{fig:lsi-uci-title} and \ref{fig:lsi-umass-title}. \
Specifically, a clear decrease in score as \emph{K} increases is observed.
...
The topic coherence scores for \emph{lda2vec} on the Title corpus exhibited a lot of noise with \
a loose negative correlations of topic coherence decreasing as \emph{K} increased demonstrated by\
...
From looking at the topic coherence scores, both graphs show a loose negative linear correlation\
of decreasing in score as \emph{K} increases. However, \ref{tab:etm_title_tc} describes \
that there is low variance for the performance of the ETM over both topic coherence metrics\
with tight bounds, showing that performance is fairly constant as \emph{K} increases which is\
a trend that the creators of the ETM observed in their original analysis \cite{dieng2019topic}.
...
When looking at the performance of all of the models on the Title corpus together, the trends\
and performance are pretty consistent across metrics shown in \ref{fig:umass-title} and \ref{fig:uci-title}.
In terms of performance the \emph{lda2vec} scored well while only producing assimilar topics. The ETM\
exhibited similar scores to \emph{lda2vec} but while producing more coherent and distinct topics.\
The LDA decreased in performance on the UMass metric, scoring noticeably lower than the word \
embedding topic models while outperforming the LSI. However, on the UCI metric the LDA showed\
a much more constant trend in topic coherence score while scoring very closely to the word embeddings\
at the top of the range. The LSI, however, scored lowest in terms of topic coherence and the performance\
decreased linearly on both metrics as \emph{K} increased.
...
The prevelance of surveys and answering questions was extremely prominent as shown in the Corpus Statistics\
...
This may be due in part to the size of the corpus and that the majority of the words
 in the Title corpus\
...
Specifically, the LSI performance on the Description corpus is very similar across metrics. This similarity\
was also observed across metric for the LSI on the Title corpus. it is intersting to observe very similar\
shapes in graph across the two metrics and a consistent negative correlation as \emph{K} increases.
...
\newpage
Based on the topic coherence performance, the LSI on the Description corpus is analyzed in greater depth in \
...
\csmfigure{lda2vec-umass-preview}{figures/lda2vec-umass-preview}{5.5in}{Preview -- \emph{lda2vec} UMass Topic Coherence}

\csmfigure{lda2vec-uci-preview}{figures/lda2vec-uci-preview}{5.5in}{Preview -- \emph{lda2vec} UCI Topic Coherence}
...
\begin{table}
	...
	\end{center}
\end{table}
\csmfigure{lda2vec-umass-dataset-comparison}{figures/lda2vec-umass-dataset-comparison}{5.5in}{\emph{lda2vec} -- UMass Topic Coherence Dataset Comparison}
\csmfigure{lda2vec-uci-dataset-comparison}{figures/lda2vec-uci-dataset-comparison}{5.5in}{\emph{lda2vec} -- UCI Topic Coherence Dataset Comparison}
\newpage
\subsubsection{ETM}
...
% it is over (see \ref{fig:Strongbad})!  Also it is important to note the placement of labels in subfigures: \ref{fig:fsm}, and \ref{fig:fsm-pirates}.
